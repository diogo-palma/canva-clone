...
export const canvasActions = {
  ...
  removeActiveObject(canvasIndex: number){
    this.canvasInstances.forEach((instance: any, index: number) => {
  ...
  async addPage(canvasId: string, move: boolean = true) {
    ...
    const handleMouseUp = () => {
      ...
      const canvasIndex = this.canvasInstances.findIndex((instance: any) => instance.canvas === fabricCanvasObj);
    ...
    const handleSelectionChanged = () => {
      fabricCanvasObj.getObjects().forEach((obj: any) => {
    ...
    fabricCanvasObj.on('object:modified', (e: any) => {
      handleObjectModified(e)
    })
    fabricCanvasObj.on('mouse:move', (e: any) => {
    ...
    fabricCanvasObj.on('mouse:out', (e: any) => {
        const target = e.target;
  ...
  addCanvasInstance(instance: { canvas: any, activeLayerIndex: number }) {
  ...
  saveCanvasState() {
    ...
    const allCanvasStates = this.canvasInstances.map((instance: any) => {
  ...
  async loadCanvasState(historyState: any) {
    ...
    const oldCountMap = new Map(oldPagesCount.map((id: any, index: number) => [id, index]));
    const newCountMap = new Map(this.pagesCount.map((id: any, index: number) => [id, index]));
      ...
      if (instance) {
        await new Promise<void>((resolve) => {
          instance.canvas.loadFromJSON(state, () => {
            instance.canvas.getObjects().forEach((obj: any) => {
  ...
  changeBlur() {
    ...
    if (activeObject && activeObject.type === 'image') {
      ...
      activeObject.filters = activeObject.filters.filter((filter: any) => !(filter instanceof fabric.Image.filters.Blur));
  ...
  changeBrightness() {
    ...
    const activeObject = canvas.getActiveObject();
    if (activeObject && activeObject.type === 'image') {
      function mapBrightness(value: number) {
        const minValue = 1;
        const maxValue = 200;
      ...
      activeObject.filters = activeObject.filters.filter((filter: any) => !(filter instanceof fabric.Image.filters.Brightness));
  ...
  changeSepia() {
    ...
    const activeObject = canvas.getActiveObject();
    if (activeObject && activeObject.type === 'image') {
      const hasSepiaFilter = activeObject.filters.some((filter: any) => filter instanceof fabric.Image.filters.Sepia);
      this.selectedSepia = hasSepiaFilter;
      if (hasSepiaFilter) {
        activeObject.filters = activeObject.filters.filter((filter: any) => !(filter instanceof fabric.Image.filters.Sepia));
      } else {
  ...
  changeGrayscale() {
    ...
    const activeObject = canvas.getActiveObject();
    if (activeObject && activeObject.type === 'image') {
      const hasGrayscaleFilter = activeObject.filters.some((filter: any) => filter instanceof fabric.Image.filters.Grayscale);

      this.selectedGrayscale = hasGrayscaleFilter
      if (hasGrayscaleFilter) {
        activeObject.filters = activeObject.filters.filter((filter: any) => !(filter instanceof fabric.Image.filters.Grayscale));
      } else {
  ...
  savePdf() {
    ...
    if (canvasInstances && canvasInstances.length > 0) {        
      ...
      canvasInstances.forEach((instance: any, index: number) => {
  ...
  async saveTextsTemplates(filename: string) {
    ...
    const textsObjs = objects.map((obj: any) => ({
      text: obj.text,
      left: obj.left,
      top: obj.top,
  ...
  async saveDesignTemplates(filename: string) {
    ...
    const textObjects = canvas.getObjects('i-text').map((obj: any) => ({
      type: 'i-text',
      text: obj.text,
      left: obj.left,
      top: obj.top,
      ...
      shadow: obj.shadow ? {
        ...
        affectStroke: obj.shadow.affectStroke
      } : null,
      index: canvas.getObjects().indexOf(obj)
    }));

    const svgObjects = canvas.getObjects('path').map((obj: any) => ({
      type: 'svg',
      src: obj.src,
      left: obj.left,
      top: obj.top,
    ...
    const imageObjects = canvas.getObjects('image').map((obj: any) => {
      const grayscaleFilter = obj.filters.find((filter: any) => filter.type === 'Grayscale');
      return {
        type: 'image',
        src: obj.getSrc(),
        left: obj.left,
  ...
  loadDesignTemplates(obj: any) {
    ...
    if (obj.backgroundImage) {
        fabric.Image.fromURL(obj.backgroundImage, (img: any) => {
            canvas.setBackgroundImage(img, canvas.renderAll.bind(canvas));
        });
    }
  
    const loadObjects = (objects: any) => {
        return Promise.all(objects.map((obj: any, index: number) => {
            return new Promise((resolve) => {
                if (obj.type === 'image') {
                    ...
                    fabric.loadSVGFromURL(obj.src, (objects: any, options: any) => {
                  ...
                  let shadow = obj.shadow ? new fabric.Shadow({
                    color: obj.shadow.color,
                    blur: obj.shadow.blur,
                    offsetX: obj.shadow.offsetX,
                    offsetY: obj.shadow.offsetY,
    ...
    loadObjects(obj.objects).then(async () => {
      const zoom = canvas.getZoom();
      
      obj.objects.forEach(async (objData: any) => {
          const adjustedLeft = Math.round(objData.left);
          const adjustedTop = Math.round(objData.top);
  
          canvas.getObjects().forEach((obj: any) => {                
              console.log("Rounded obj.left:", Math.round(obj.left), "Rounded obj.top:", Math.round(obj.top));
          });
  
          const fabricObject = canvas.getObjects().find((obj: any) => 
              Math.round(obj.left) === adjustedLeft && 
              Math.round(obj.top) === adjustedTop
          );
  
          if (fabricObject) {
              fabricObject.moveTo(objData.index);
          }
      });
      canvas.renderAll();
      this.saveCanvasState();
  ...
  addLayer(canvasIndex: number, object: any) {
  ...
  checkObjectType(obj: any) {      
    ...
    if (activeObjects.length === 1) {
      ...
      function unmapBrightness(mappedValue: number) {
        const minValue = 1;
        const maxValue = 200;
        const minMappedValue = -1;
        const maxMappedValue = 1;
      ...
      if (activeObject.filters) {
        activeObject.filters.forEach((filter: any) => {
  ...
  setZoom(applyZoom = true) {
    ...
    this.canvasInstances.forEach((obj: any) => {
  ...
  checkPosition(canvas: any, activeObject: any){
    ...
    const canvasIndex = this.canvasInstances.findIndex((instance: any) => instance.canvas === canvas);
    this.setActiveLayer(canvasIndex, layerIndex)
  ...
  pasteObject() {
    ...
    if (this.clonedObject) {
      this.clonedObject.clone((clonedObj: any) => {
        canvas.discardActiveObject();
        clonedObj.set({
